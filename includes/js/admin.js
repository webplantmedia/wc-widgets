...
(function($) {
	...
	$body.on('click', '.wc-widgets-image-upload', function( event ) {
		...
		if ( 'wc_widgets_insert_single' === options.state ) {
			// add the single insert state
			file_frame[frame_key].states.add([
				// Main states.
				new media.controller.Library({
					id:         'wc_widgets_insert_single',
		...
		else if ( 'wc_widgets_insert_multi' === options.state ) {
			// add the single insert state
			file_frame[frame_key].states.add([
				new media.controller.Library({
					id:         'wc_widgets_insert_multi',
					title: clicked.data( 'title' ),
					priority:   20,
					toolbar:    'select',
					filterable: 'uploaded',
	...
	.on('click', '.wc-widgets-restore-image', function( e ) {
		e.preventDefault();
